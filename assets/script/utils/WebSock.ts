...
import { ISocket, MessageFunc, NetData } from "../entity/network/NetInterface";
...
export class WebSock implements ISocket {
    ...
    connect(options: any) {
        ...
        if (window.WebSocket) {
            ...
            this._ws.onopen = e => {
                Logger.logNet("websocket connected...");
                Logger.logNet("userAgent: ", navigator.userAgent)
                this._ws.send(options.userAddress);
            }
            this._ws.onerror = this.onError;
            this._ws.onclose = this.onClosed;
            if (this._ws) {
                if (this._ws.readyState === WebSocket.CONNECTING) {
                    Logger.logNet("websocket connecting...");
                }
            }
            return true;
        } else {
            Logger.logNet("websocket not support...");
            return false;
        }
    }
