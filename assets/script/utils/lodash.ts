...
export class lodash {
     ...
     *@Author: yozora
     *@Description: 格式化时间戳
     *@Date: 2022-01-11 14:31:46
     */
    public static formatTimestamp(timestamp: Date, format: string = 'hh:mm:ss') {
        var o = {
     ...
     *@Author: yozora
     *@Description: 格式化秒
     *@Date: 2021-11-18 13:57:05
     */
    public static formatSeconds(value: string) {
        ...
        let result: string = "";
        if (theTime > 0) {
            result = "" + lodash.check(theTime.toFixed(0));
        }
        if (theTime == 0) {
            result = "00";
        }
        if (theTime1 > 0) {
            result = "" + lodash.check(theTime1.toFixed(0)) + " : " + result;
        }
        if (theTime1 == 0) {
            result = "00" + " : " + result;
        }
        if (theTime2 > 0) {
            result = "" + lodash.check(theTime2.toFixed(0)) + " : " + result;
        }
        if (theTime2 == 0) {
            result = "00" + " : " + result;
        }
    ...
    private static check(val: string | number) {
        if (val < 0) {
            return "00";
        }
        if (val < 10) {
    ...
    /*
     *@Author: yozora
     *@Description: 按结束时间倒计时
     *@Date: 2022-02-13 17:24:09
     */
    public static countdown(endtime: string) {
        const total = Date.parse(endtime) - Date.parse(new Date().toString());
        const seconds = this.check(Math.floor((total / 1000) % 60));
        const minutes = this.check(Math.floor((total / 1000 / 60) % 60));
        const hours = this.check(Math.floor((total / (1000 * 60 * 60)) % 24));
        const days = this.check(Math.floor(total / (1000 * 60 * 60 * 24)));
