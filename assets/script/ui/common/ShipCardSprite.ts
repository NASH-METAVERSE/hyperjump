...
@ccclass('ShipCardSprite')
export class ShipCardSprite extends Component {
    ...
    */
    ...
    onEnable() {
        ClientEvent.on(GameConstans.CLIENTEVENT_LIST.CHANGE_CARD_HOVER_STATUS, this.onHoverCard, this);
        ClientEvent.on(GameConstans.CLIENTEVENT_LIST.CHANGE_CARD_PRESSED_STATUS, this.onCheckedCard, this);
    }
    ...
    private changeCardSprite(shipCardName: string, isShow: boolean, type: number) {
        if (isShow) {
            ...
            if (type === 0) {
                ...
                // }
                // const anime = this.node.getChildByName(shipCardName).getComponent(Animation);
                // if (anime.getState("card_grow") !== null && !anime.getState("card_grow").isPlaying) {
                //     anime.play("card_grow");
                // }
            }
        } else {
            ...
            if (type !== 0) {
                // 重置刻度栏状态
                // if (this.node.parent.getChildByName("Ship_list_border").getComponent(Animation).getState("dail_grow").isPlaying) {
                //     this.node.parent.getChildByName("Ship_list_border").getComponent(Animation).getState("dail_grow").stop();
                // }
        ...
        if (shipCardName !== null) {
            this.changeShipStatusSprite(shipCardName, "Ship_status_area_1", isShow);
            this.changeShipStatusSprite(shipCardName, "Ship_status_area_2", isShow);
        }
